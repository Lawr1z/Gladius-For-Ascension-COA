...
function Gladius:SetupOptions()
		...
		args = {
            ...
            args = {
               ...
               ascensionStun = {
                  type="toggle",
                  name="Ascension Stun",
                  desc="Track Ascension-only stun DR category",
                  order=170,
                  hidden=function() return not (Gladius.AscensionDRCategories and Gladius.AscensionDRCategories["ASCENSION_STUN"]) end,
                  get=function(info) return self.db.profile.drList["ASCENSION_STUN"] == nil and true or self.db.profile.drList["ASCENSION_STUN"] end,
                  set=function(info, value) self.db.profile.drList["ASCENSION_STUN"] = value end,
               },
               ascensionFear = {
                  type="toggle",
                  name="Ascension Fear",
                  desc="Track Ascension-only fear DR category",
                  order=171,
                  hidden=function() return not (Gladius.AscensionDRCategories and Gladius.AscensionDRCategories["ASCENSION_FEAR"]) end,
                  get=function(info) return self.db.profile.drList["ASCENSION_FEAR"] == nil and true or self.db.profile.drList["ASCENSION_FEAR"] end,
                  set=function(info, value) self.db.profile.drList["ASCENSION_FEAR"] = value end,
               },
               ascensionTimestop = {
                  type="toggle",
                  name="Ascension Time Stop",
                  desc="Track Chronomancer time-stop DR category",
                  order=172,
                  hidden=function() return not (Gladius.AscensionDRCategories and Gladius.AscensionDRCategories["ASCENSION_TIMESTOP"]) end,
                  get=function(info) return self.db.profile.drList["ASCENSION_TIMESTOP"] == nil and true or self.db.profile.drList["ASCENSION_TIMESTOP"] end,
                  set=function(info, value) self.db.profile.drList["ASCENSION_TIMESTOP"] = value end,
               },
            },
         },
		}
	...
	for class, spells in pairs(self.cooldownSpells) do
      local customClass = self.AscensionClasses and self.AscensionClasses[class]
      self.options.args.cooldowns.args[class] = {
         type = "group",
         order = 1,
         name = LOCALIZED_CLASS_NAMES_MALE[class] or (customClass and customClass.name) or tostring(class),
         args = {
            cooldownToggle = {
               type="toggle",
      ...
      for k,v in pairs(self.cooldownSpells[class]) do
         self.options.args.cooldowns.args[class].args[tostring(k)] = {
            type = "group",
            order = 1,
            name = GetSpellInfo(k) or ("Spell " .. tostring(k)),
            args = {
               cooldownToggle = {
                  type="toggle",
                  name=L["Show cooldown"],
