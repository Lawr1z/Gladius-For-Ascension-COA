...
local function CastUpdate(self, elapsed)
	-- Ascension: school lockout overlay after interrupt
	if (self.isLockout and self.lockoutEnds) then
		local remaining = self.lockoutEnds - GetTime()
		if (remaining <= 0) then
			self.isLockout = nil
			self.lockoutEnds = nil
			Gladius:CastEnd(self)
			local pdb = db or (Gladius.db and Gladius.db.profile)
			if (pdb and pdb.castBarColor) then
				self:SetStatusBarColor(pdb.castBarColor.r, pdb.castBarColor.g, pdb.castBarColor.b, pdb.castBarColor.a)
			end
			return
		end
	...
	for i=1, currentBracket do
      ...
      elseif (db.castBarPos == "LEFT") then
		...
		if(button.class and db.healthBarClassColor) then
			local hr, hg, hb = Gladius:GetClassColor(button.class)
