...
function Gladius:OnInitialize()
	local ok, database = pcall(function()
		return LibStub("AceDB-3.0"):New("GladiusDB", self:getDefaults())
	end)
	if (not ok or not database) then
		DEFAULT_CHAT_FRAME:AddMessage("|cffff3333Gladius initialization error:|r " .. tostring(database))
		return
	end
	self.db = database
	...
	-- Project Ascension: merge custom classes / auras / DR / interrupts
	if (self.MergeAscensionData) then
		self:MergeAscensionData()
	end
	...
	if (arenaUnits[unit]) then
		...
		if(not UnitIsDeadOrGhost(unit)) then
			...
			if ( db.lowHealthAnnounce and healthPercent <= db.lowHealthPercentage and not button.lowHealth and (not button.healthThrottle or GetTime() > button.healthThrottle) and button.name ) then
				...
				local ar, ag, ab = self:GetClassColor(button.class)
				Gladius:SendAnnouncement(text, { r = ar, g = ag, b = ab })
	...
	if ( arenaUnits[unit] == "playerUnit" ) then	
		...
		while ( true ) do
            ...
            if((db.auraAnnounce or (db.auraAnnounceList[auraName] and db.auraAnnounceList[auraName] ~= "disabled")) and (not button.lastAuraName or auraName ~= button.lastAuraName)) then
               local _, auraClass = UnitClass(unit)
               local ar, ag, ab = self:GetClassColor(auraClass)
               self:SendAnnouncement(string.format(L["AURA GAIN: %s (%s) - %s for %s seconds"], UnitName(unit), UnitClass(unit), auraName, ceil(auraExpTime - GetTime())), { r = ar, g = ag, b = ab }, db.auraAnnounceList[auraName] and db.auraAnnounceList[auraName] or db.announceType)
			...
			if ( name == DRINK_SPELL and db.drinkAnnounce and ( not button.drinkThrottle or GetTime() > button.drinkThrottle )) then
				local ar, ag, ab = self:GetClassColor(button.class)
				Gladius:SendAnnouncement(string.format(L["DRINKING: %s (%s)"], button.name, button.classLoc), { r = ar, g = ag, b = ab })
...
for _, spellId in ipairs({ 48950, 48171, 49277, 50763 }) do
	local spellName = GetSpellInfo(spellId)
	if spellName then
		RESURRECTION_SPELLS[spellName] = true
	end
end
function Gladius:UNIT_SPELLCAST_START(event, unit)
	...
	if (arenaUnits[unit] == "playerUnit" and spell) then
		...
		if(RESURRECTION_SPELLS[spell] and db.resAnnounce) then
			local ar, ag, ab = self:GetClassColor(button.class)
			Gladius:SendAnnouncement(string.format(L["RESURRECTING: %s (%s)"], UnitName(unit), UnitClass(unit)), { r = ar, g = ag, b = ab })
		end
	end
end
...
function Gladius:DetectSpec(unit, spec)
	...
	if (db.cooldown) then
      ...
      local classCDs = class and self.cooldownSpells and self.cooldownSpells[class]
      if (classCDs) then
         for k,v in pairs(classCDs) do
	...
	if( db.specAnnounce ) then
		local _, announceClass = UnitClass(unit)
		local ar, ag, ab = self:GetClassColor(announceClass)
		self:SendAnnouncement(string.format(L["SPEC DETECTED: %s - %s %s"], UnitName(unit), spec, UnitClass(unit)), { r = ar, g = ag, b = ab })
	end
	...
	if (arenaUnits[unit] == "playerUnit" and db.targetIcon) then
      local button = self.buttons[unit]
		...
		if (targetClass ~= nil) then
			if (self.SetClassIconTexture) then
				self:SetClassIconTexture(targetIcon, targetClass)
			elseif (CLASS_BUTTONS[targetClass]) then
...
function Gladius:COMBAT_LOG_EVENT_UNFILTERED(event, timestamp, eventType, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, ...)
   ...
   elseif (eventType == "SPELL_AURA_REFRESH") then
	...
	elseif ( eventType == "SPELL_INTERRUPT" ) then
		-- Ascension: show school lockout on the interrupted unit's cast bar
		local spellID, spellName, spellSchool, extraSpellID, extraSpellName, extraSchool = ...
		local unit = arenaGUID[destGUID]
		if (unit and self.AscensionApplyInterruptLockout) then
			self:AscensionApplyInterruptLockout(unit, spellID)
		end
	end
...
function Gladius:CooldownUsed(unit, unitClass, spellId, spellName)
   ...
   if (not unitClass or not self.cooldownSpells[unitClass] or not self.cooldownSpells[unitClass][spellId]) then return end
   ...
   if (db.cooldownAnnounce or db.cooldownAnnounceList[spellId] or db.cooldownAnnounceList[unitClass]) then
      local ar, ag, ab = self:GetClassColor(unitClass)
      self:SendAnnouncement(string.format(L["COOLDOWN USED: %s (%s) used %s - %s sec. cooldown"], UnitName(unit), UnitClass(unit), spellName, cd), { r = ar, g = ag, b = ab }, db.cooldownAnnounceList[spellId] and db.cooldownAnnounceList[spellId] or db.announceType)
   end
...
local function TrinketUpdate(self, elapsed)
	if ( self.endTime < GetTime() ) then	
		...
		if ( db.trinketUpAnnounce ) then
			local ar, ag, ab = Gladius:GetClassColor(button.class)
			Gladius:SendAnnouncement(string.format(L["TRINKET READY: %s (%s)"], button.name, button.classLoc), { r = ar, g = ag, b = ab })
		end
...
function Gladius:TrinketUsed(unit, time)
	...
	if ( db.trinketUsedAnnounce ) then
		local ar, ag, ab = self:GetClassColor(button.class)
		local color = { r = ar, g = ag, b = ab }
		if (time == 120) then
			Gladius:SendAnnouncement(string.format(L["TRINKET USED: %s (%s)"], button.name, button.classLoc), color)
		else
			Gladius:SendAnnouncement(string.format("WOTF USED: %s (%s)", button.name, button.classLoc), color)
		end
	end
end
	...
	if (UnitExists(unit)) then
      if (arenaUnits[unit] == "playerUnit" or (arenaUnits[unit] ~= "playerUnit" and db.showPets)) then
		...
		if ( db.enemyAnnounce and not button.enemyAnnounced and name ~= "Unknown" ) then
			local ar, ag, ab = self:GetClassColor(class)
			Gladius:SendAnnouncement(name .. " - " .. classLoc, { r = ar, g = ag, b = ab })
		...
		--health bar class color (safe for Ascension custom classes)
		local hr, hg, hb = self:GetClassColor(class)
		button.health:SetStatusBarColor(hr, hg, hb, 1)
		
		--class icon (safe for Ascension: no unpack(nil) on CLASS_BUTTONS)
		if (self.SetClassIconTexture) then
			self:SetClassIconTexture(button.classIcon, class)
		elseif (class and CLASS_BUTTONS[class]) then
			...
			button.classIcon:SetTexture("Interface\\Icons\\INV_Misc_QuestionMark")
		...
		if (db.cooldown and class and self.cooldownSpells[class]) then         
...
function Gladius:Test()
	...
	for i=1, Gladius.currentBracket do
		...
		if ( db.enemyAnnounce ) then
			local ar, ag, ab = self:GetClassColor(class)
			Gladius:SendAnnouncement(button.name .. " - " .. classLoc, { r = ar, g = ag, b = ab })
		end
